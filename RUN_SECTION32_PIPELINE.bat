...
if not defined DATABOSS_PROJECT_ROOT (
  echo ERROR: Set DATABOSS_PROJECT_ROOT to the mounted Section 32 evidence folder.
  exit /b 2
)
...
".venv\Scripts\python.exe" -m databoss_title_factory pipeline --root "%DATABOSS_PROJECT_ROOT%" --allow-root "%DATABOSS_PROJECT_ROOT%" >>".runtime\logs\section32-pipeline.log" 2>&1
exit /b %ERRORLEVEL%
