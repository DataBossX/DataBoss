...
def extractor_llm(text: str, source_url: str) -> dict[str, Any]:
    ...
    if m:
        payload["doc_no"] = m.group(1)
    if "OIL & GAS LEASE" in text.upper():
        payload["instrument"] = "Oil & Gas Lease"
    m = re.search(r"(20\d{2}-\d{2}-\d{2})|(\d{2}/\d{2}/20\d{2})", text)
    if m:
        payload["recording_date"] = m.group(0).replace("/", "-")
    m = re.search(r"Sec(tion)?\s*1.*T7N.*R63W", text, re.I)
    if m:
        payload["legal_short"] = "Sec 1 T7N R63W"
    return payload
