...
def decide_status_rule_based(owner: str, docs: list[dict]) -> dict:
    if not owner or not str(owner).strip():
        return {"status": "Unknown—needs manual", "confidence": 0.0, "explanation": "No owner provided."}
    if not docs:
        return {"status": "Unknown—needs manual", "confidence": 0.5, "explanation": "No documents provided."}
    ...
    if "LEASE" in instr and o == grantor:
        return {
            "status": "Leased HBP",
            "confidence": 0.7,
            "explanation": "Owner is lessor on O&G lease for Sec 1; no later deed out recorded.",
        }
    if o == grantor:
        return {
            "status": "Assigned out",
            "confidence": 0.85,
            "explanation": "Latest affecting doc shows owner as grantor/assignor in Sec 1.",
        }
    if o in grantees:
        return {
            "status": "Current owner of record",
            "confidence": 0.85,
            "explanation": "Latest affecting doc shows owner as grantee for Sec 1 with no later conveyance out.",
        }
