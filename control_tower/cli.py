...
from .reconcile import reconcile_authority
...
def main(argv=None):
    ...
    parser.add_argument(
        "command",
        choices=("selftest", "canary", "audit", "reconcile"),
        help="what to run",
    )
    parser.add_argument(
        "--run-root",
    ...
    if args.command == "reconcile":
        report = reconcile_authority(client=drive_client)
        path, digest = _emit(report, run_dir, "AUTHORITY_RECONCILIATION_REPORT.json")
        print("governing ruling: {0}".format(report["controlling_owner_rulings"][0]["ruling_id"]))
        print("governing posture: {0}".format(report["governing_posture"]))
        print("spent commands:    {0}".format(len(report["retired_and_spent_commands"])))
        print("report: {0}".format(path))
        print("sha256: {0}".format(digest))
        print("hold:   {0}".format(HOLD))
        return 0
