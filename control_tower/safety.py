...
def make_sha256_sidecar(filename, payload_bytes):
    """Generate a reproducible .sha256 sidecar string."""
    digest = sha256_hex(payload_bytes)
    base_name = filename.split("/")[-1].split("\\")[-1]
    return "{0}  {1}\n".format(digest, base_name)


def assert_active_command(file_id, command_id=None):
    """Verify that a command is not retired or spent."""
    if file_id in SPENT_COMMAND_DRIVE_IDS:
        raise RetiredCommandDenied(
            "refusing spent command by Drive ID {0}".format(file_id)
        )
    if command_id and command_id in SPENT_COMMAND_IDS:
        raise RetiredCommandDenied(
            "refusing spent command by command ID {0}".format(command_id)
        )
    return True


def assert_output_allowed(target, allowed_outputs=None):
    """Fail closed unless target is in allowed_outputs or allowed write folders."""
    if allowed_outputs is not None:
        if target not in allowed_outputs:
            raise OutputNotAllowed(
                "target {0} not in output allowlist: {1}".format(target, sorted(allowed_outputs))
            )
    elif target not in ALLOWED_WRITE_FOLDER_IDS:
        raise OutputNotAllowed(
            "target {0} not in allowed write folders".format(target)
        )
    return target
