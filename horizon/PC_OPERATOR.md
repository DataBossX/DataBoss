...
3. When `--receipt-dir` is set (with or without `--execute`), writes
   ...
   `title-opinion-letter-p15.xlsx` and substring hits such as
   `temp15.xlsx` are ignored. Those leftover exclusive isolated
   filenames win section detection over the parent folder and are not
   picked as source workbooks. A Drive Isolated/ copy of that leftover exclusive
...
5. With `--execute`, those isolated hops actually run into `--receipt-dir`.
   ...
   is absent or stale. After Phase 2, that match uses the authorized
   source-document faces, not every PDF under the snapshot root
   (authorized typed or handwritten index PDFs stay out of the census
   file set). A stale conventional census is unbound so inventory can
   rewrite it from the live faces.
