...
`index` is a PDF or workbook; a notes file that only mentions index is
not. Root labels must identify their source kind: `pc`, `drive`, `chat`,
or a suffixed form such as `drive_2`.
