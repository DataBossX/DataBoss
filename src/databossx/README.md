...
| `title_extraction.py` | Cursory **conveyance extraction** from raw document text (deeds/assignments/leases): grantor/grantee, instrument, conveyed interest, legal (via `legal.py`), acres. Wired as the chain **fallback** so a folder of loose documents (no OGL workbook) still yields a reconciled chain. Every field nullable; unfound ⇒ blank + review flag. |
