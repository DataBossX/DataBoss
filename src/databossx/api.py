...
<div id="title" class="view"><p>Import operator-reviewed records only. Every reviewed instrument must cite an evidence asset ID and exact character range.</p><textarea id="title-json" placeholder='{"name":"Title case","legal_description":"...","gross_acres":{"numerator":160,"denominator":1},"opening_ownership":[],"instruments":[]}'></textarea><button onclick="createTitleCase()">Import reviewed title case</button><div id="title-cases"></div></div>
...
async function refresh(){if(!active)return;const [assets,runs,audit,cases]=await Promise.all([api(`/projects/${active}/assets`).then(r=>r.json()),api(`/projects/${active}/runs`).then(r=>r.json()),api(`/projects/${active}/audit`).then(r=>r.json()),api(`/projects/${active}/title-cases`).then(r=>r.json())]);
...
async function reviewTitle(id,hash){const reviewer=prompt('Qualified reviewer name:');if(!reviewer)return;if(!confirm('Approve this exact package hash? This does not certify an abstract or title opinion.'))return;try{await api(`/title-packages/${id}/review`,{method:'POST',body:JSON.stringify({package_manifest_sha256:hash,reviewer,decision:'APPROVE',notes:'Approved in local Command Center'})});await refresh();busy('Exact package hash approved for export')}catch(e){alert(e.message)}}
