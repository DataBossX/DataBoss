...
class EvidenceGapPackageTests(unittest.TestCase):
    ...
    def test_builds_complete_validated_package(self):
        build(self.output, self.project, "2026-07-15T00:00:00+00:00")

        for name in (*OUTPUT_NAMES, "SHA256SUMS.txt", "INVOCATION_MANIFEST.json"):
    ...
    def test_escapes_formula_like_client_input(self):
        project = dict(self.project, client="=HYPERLINK(\"https://example.test\")")
        build(self.output, project, "2026-07-15T00:00:00+00:00")
    ...
    def test_refuses_repository_public_output(self):
        repository_output = (
        ...
        with self.assertRaisesRegex(ValueError, "ignored private directory"):
            build(repository_output, self.project, "2026-07-15T00:00:00+00:00")

    def test_refuses_to_overwrite_package(self):
        build(self.output, self.project, "2026-07-15T00:00:00+00:00")
